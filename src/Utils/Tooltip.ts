...
import {useDebounce} from "self://Utils/Misc";
...
export const useTooltip = (classBase: string) => {
    ...
    //let { type, open, targetEl, tooltipEl } = useDebounce(tooltipState, 100);
    ...
    let attributes = usePopperAttributes({
        classBase, type, open, initialized: targetEl && tooltipEl, popper,
    });
    ...
    let setTooltip = useCallback((el) => {
        if (el) {
            setTooltipState(s => ({...s, tooltipEl: el}))
        }
    }, [setTooltipState])

    let showTooltip = useCallback((target: Node, timeout?: number, type?: string, cb?: () => any) => {
        if (tooltipTimeout) {
